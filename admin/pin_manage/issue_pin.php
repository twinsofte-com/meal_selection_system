...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $role = 'issue'; // Role is fixed for this file

  if (isset($_POST['update_id'])) {
    $id = $_POST['update_id'];
    $stmt = $conn->prepare("UPDATE issue_pins SET pin_code = ? WHERE id = ?");
    $stmt->bind_param("si", $pin_code, $id);
  } else {
    $stmt = $conn->prepare("INSERT INTO issue_pins (role, pin_code) VALUES (?, ?)");
    $stmt->bind_param("ss", $role, $pin_code);
  }

  $stmt->execute();
  header("Location: issue_pin.php");
  exit();
}
...
if (isset($_GET['delete'])) {
  ...
  $conn->query("DELETE FROM issue_pins WHERE id = $id");
  header("Location: issue_pin.php");
  exit();
}

// Fetch all pins
$pins = $conn->query("SELECT * FROM issue_pins ORDER BY id DESC");
?>
...
<body class="p-6 bg-gray-50">
  ...
  <h1 class="text-2xl font-bold text-[#2E3095] mb-4">Manage issue PINs</h1>

  <form method="POST" class="mb-6 space-y-2">
    <!-- Hidden Role -->
    <input type="hidden" name="role" value="issue">
    <!-- PIN Input -->
