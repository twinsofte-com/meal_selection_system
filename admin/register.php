...
<div class="p-6">
  ...
  <button onclick="openRegisterModal()" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-700">+ Register Staff</button>

  <div class="mt-6 bg-white rounded shadow p-4">
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-xl font-semibold text-[#2E3095]">Registered Staff</h2>
      <input type="text" id="searchInput" placeholder="Search Staff..." class="border rounded p-2 w-64" onkeyup="filterStaff()" />
    </div>
          ...
          <tr class="border-t">
            ...
            <td class="p-2">
              <?php if ($row['qr_code']): ?>
                <?= $row['qr_code'] ?>
              <?php else: ?>
                <button onclick="openQrUploadModal(<?= $row['id'] ?>)" class="text-[#2E3095] text-2xl font-bold">+</button>
              <?php endif; ?>
            </td>
            <td class="p-2">
              <button onclick='openEditModal(<?= json_encode($row) ?>)' class="bg-yellow-500 text-white px-2 py-1 rounded text-sm mr-2 hover:bg-yellow-600">Edit</button>
              <button onclick="openDeleteModal(<?= $row['id'] ?>)" class="bg-red-500 text-white px-2 py-1 rounded text-sm hover:bg-red-600">Delete</button>
            </td>
  ...
  <div class="bg-white rounded-xl w-full max-w-md p-6 relative">
    ...
    <form method="POST" action="register.php" onsubmit="combineStaffID()">
      ...
      <div class="mb-4">
        ...
        <select name="staff_type" id="staff_type" onchange="updatePrefix()" required class="w-full border p-2 rounded-lg">
  ...
  <div class="bg-white rounded-xl w-full max-w-md p-6 relative">
    ...
    <form method="POST" action="upload_qr_manual.php">
      ...
      <div class="mb-4">
        ...
        <input type="text" id="qr_text" name="qr_text" required class="mt-1 block w-full border p-2 rounded-md focus:ring focus:border-blue-300" placeholder="2025-225">
      </div>
...
<!-- Edit Modal -->
<div id="editModal" class="<?= isset($_GET['error']) ? '' : 'hidden' ?> fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
  <div class="bg-white rounded-xl w-full max-w-md p-6 relative">
    ...
    <?php if (isset($_GET['error'])): ?>
      <div class="mb-4 p-3 bg-red-100 text-red-700 rounded">
        <?php if ($_GET['error'] === 'duplicate_qr'): ?>
          This QR text is already in use by <strong><?= htmlspecialchars($_GET['conflict_name']) ?></strong>. Please enter a unique one.
    ...
    <form method="POST" action="edit_staff.php">
      ...
      <div class="mb-3">
        <label class="block text-gray-700">Name</label>
        <input type="text" name="name" id="edit_name" required class="w-full border p-2 rounded" value="<?= htmlspecialchars($_GET['name'] ?? '') ?>" />
      </div>
      <div class="mb-3">
        <label class="block text-gray-700">Phone</label>
        <input type="text" name="phone_number" id="edit_phone" required class="w-full border p-2 rounded" value="<?= htmlspecialchars($_GET['phone'] ?? '') ?>" />
      </div>
      <div class="mb-3">
        <label class="block text-gray-700">Staff Type</label>
        <input type="text" name="staff_type" id="edit_type" required class="w-full border p-2 rounded" value="<?= htmlspecialchars($_GET['type'] ?? '') ?>" />
      </div>
      <div class="mb-3">
        <label class="block text-gray-700">QR Code</label>
        <input type="text" name="qr_text" id="edit_qr" required class="w-full border p-2 rounded" value="<?= htmlspecialchars($_GET['qr'] ?? '') ?>" />
      </div>
  ...
  <div class="bg-white rounded-xl w-full max-w-sm p-6 relative text-center">
    ...
    <form method="POST" action="delete_staff.php">
      ...
      <div class="flex justify-center space-x-4 mt-4">
        ...
        <button type="button" onclick="closeDeleteModal()" class="bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500">Cancel</button>
      </div>
    </form>
...
<script>
  ...
  function openEditModal(staff) {
    ...
    document.getElementById('edit_type').value = staff.staff_type;
