...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pin_code'])) {
    ...
    if (!empty($pin_code)) {
        $stmt = $conn->prepare("INSERT INTO issue_pins (role, pin_code) VALUES ('issue', ?)");
        $stmt->bind_param("s", $pin_code);
        $stmt->execute();
        $msg = "New issue PIN added successfully!";
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM issue_pins WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $msg = "Issue PIN deleted successfully!";
}

// Fetch all issue PINs
$result = $conn->query("SELECT * FROM issue_pins WHERE role = 'issue' ORDER BY id DESC");
?>
...
<div class="max-w-3xl mx-auto p-6 mt-6 bg-white shadow rounded">
  <h2 class="text-2xl font-bold mb-4">Issue PIN Management</h2>
  ...
  <form method="POST" class="mb-6">
    <label class="block mb-2 font-medium">New Issue PIN</label>
    <div class="flex">
    ...
    <table class="w-full table-auto border border-gray-300">
      ...
      <tbody>
        <?php if ($result->num_rows > 0): $i = 1; ?>
          ...
          <?php endwhile; ?>
        <?php else: ?>
          <tr><td colspan="3" class="px-4 py-4 text-center text-gray-500">No issue PINs available.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
